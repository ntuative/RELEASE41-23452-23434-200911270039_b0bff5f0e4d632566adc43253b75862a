   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(var_669 == const_757)
            {
               throw var_1464;
            }
            return var_669 == const_486;
         }
         return false;
      }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!_disposed)
         {
            _loc4_ = var_268[param1];
            if(_loc4_)
            {
               _loc5_ = 0;
               for each(_loc6_ in _loc4_)
               {
                  if(_loc6_.callback == param2 && _loc6_.var_2132 == param3)
                  {
                     _loc4_.splice(_loc5_,1);
                     _loc6_.callback = null;
                     if(_loc4_.length == 0)
                     {
                        delete var_268[param1];
                     }
                     return;
                  }
                  _loc5_++;
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         else
         {
            ...
            {
               if(_loc8_.callback == param2 && _loc8_.var_2132 == param3)
               {
                  return;
               }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_268[event.type];
         if(array)
         {
            callbacks = [];
         ...
         var_669 = !!event.isDefaultPrevented() ? uint(const_1087) : uint(const_486);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_268)
            {
               _loc2_ = var_268[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete var_268[_loc1_];
            }
            var_268 = null;
            var_69 = null;
            _disposed = true;
