package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_385 >= var_286.length)
         {
            return "";
         }
         var petColor:IPetColor = var_286[var_385] as IPetColor;
         var color:uint = petColor.rgb;
         var p:String = _name + String.fromCharCode(10) + addZeroPadding(String(var_1388),3) + String.fromCharCode(10) + addZeroPadding(color.toString(16).toUpperCase(),6);
         return p;
      }
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_385 = param1.index;
         if(var_385 < 0 || var_385 > var_286.length)
         {
            var_385 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_894.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_227,onTextWindowEvent);
         var_894 = new Map();
         ...
         if(page && false && page.viewer.catalog)
         {
            ...
            if(_loc10_ && _loc10_.avatarRenderManager && _loc10_.avatarRenderManager.petDataManager)
            {
               ...
               if(_loc11_)
               {
                  var_948 = _loc11_.breeds.length;
                  var_286 = _loc11_.colors;
                  if(var_286 == null)
                  {
                     var_286 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_948)
         {
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_779,_loc6_),getRaceLocalizationKey(var_779,_loc6_)));
            _loc6_++;
         }
         var_894.add(var_779,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_795,onApproveNameResult);
         _loc5_ = var_894.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_286)
         {
      ...
      {
         var _loc6_:* = null;
         var _loc1_:Offer = var_894.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_385 >= 0 && var_385 < var_286.length)
         {
            _loc6_ = var_286[var_385] as IPetColor;
            if(_loc6_ != null)
            {
               _loc2_ = uint(_loc6_.rgb);
            }
         }
         var _loc3_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_779,var_1388,_loc2_,AvatarScaleType.const_50);
         if(_loc3_ == null)
         {
            return;
         }
         _loc3_.initActionAppends();
         _loc3_.appendAction(AvatarAction.const_309,AvatarAction.const_645);
         _loc3_.endActionAppends();
         _loc3_.setDirection(AvatarSetType.const_38,2);
         setPreviewImage(_loc3_.getCroppedImage(AvatarSetType.const_38).clone());
         _loc3_.dispose();
         var _loc4_:ICoreLocalizationManager = (page.viewer.catalog as HabboCatalog).localization;
